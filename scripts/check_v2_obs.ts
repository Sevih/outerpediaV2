...
console.log('  # | char    | slot | crit | adv  | xATK | tgt        | obs   | calc  | ratio')
console.log('----+---------+------+------+------+------+------------+-------+-------+------')
for (let i = 0; i < observations.length; i++) {
  const o = observations[i] as Record<string, never> & {
    ...
    mode?: string; monsterId?: string; charFlags?: { umeActive?: boolean; sakuraActive?: boolean };
  ...
  const ctx: RecomputeContext = {
    ...
    externalBuffs: o.externalBuffs, bossMechanics: o.bossMechanics,
  }
  ...
  console.log(`  ${(i+1).toString().padStart(2)} | ${o.charName.padEnd(7)} | ${slot} | ${crit} | ${elem} | ${xAtk.padEnd(4)} | ${tgt} | ${o.obs.toString().padStart(5)} | ${r.calculated.toString().padStart(5)} | ${ratio}`)
}
