...
function SkillCard({ skill, lang }: { skill: BossSkill; lang: Lang }) {
  ...
  return (
    <div className={`rounded-lg border p-3 ${isPassive ? 'border-amber-500/30 bg-amber-950/20' : 'border-white/5 bg-neutral-900/50'}`}>
...
function BossCard({ boss, lang }: { boss: Boss; lang: Lang }) {
  ...
  return (
    <div className="space-y-2">
      {/* Header */}
      <div className="flex items-center gap-3 rounded-lg border border-white/10 bg-neutral-900/70 p-3">
...
export default function WorldBossDisplay({ config, defaultMode = 'Extreme', preloadedBosses }: Props) {
  ...
  useEffect(() => {
    // Skip loading for default mode when preloaded data is available
    if (mode === defaultMode && (preloadedBoss1 || preloadedBoss2)) return;
