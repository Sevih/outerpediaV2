...
export default function StatsRankingSection({ character, ee }: Props) {
  const { t } = useI18n();
    ...
    <section id="stats-ranking">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-white/10 bg-zinc-900/60 p-4">
          <h3 className="mb-3 text-sm font-semibold text-zinc-300">{t('page.character.stats.title')}</h3>
          <p className="text-sm italic text-zinc-500">{t('page.character.stats.coming_soon')}</p>
        </div>

        {/* Ranking */}
        <div className="rounded-xl border border-white/10 bg-zinc-900/60 p-4">
          <h3 className="mb-3 text-sm font-semibold text-zinc-300">{t('page.character.toc.ranking')}</h3>
