...
export default function CharactersFiltersDrawer({
  ...
  onResetAll, matchCount, totalCount,
}: Props) {
  const { t } = useI18n();
  ...
  return (
    <div className="fixed inset-0 z-70 flex items-end md:items-stretch md:justify-end" role="dialog" aria-modal="true">
      {/* Scrim */}
      <button
        type="button"
        aria-label={t('characters.filters.close')}
        onClick={onClose}
        className={`absolute inset-0 cursor-default bg-black/60 backdrop-blur-xs transition-opacity duration-200 ${entered ? 'opacity-100' : 'opacity-0'}`}
      />
      ...
      >
        ...
        <div className="flex items-center gap-3 border-b border-zinc-800/80 px-5 py-3.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <Eyebrow className="mt-0.5 block">{t('characters.filters.advanced.subtitle')}</Eyebrow>
          </div>
          <button
            type="button"
            onClick={onResetAll}
