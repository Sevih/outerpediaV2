...
export default function TierListPveClient({ characters }: Props) {
  ...
  const payload = useMemo<FilterPayload>(() => ({
    ...
    tr: transcendLevel !== 6 ? transcendLevel : undefined,
  }), [elementFilter, classFilter, rarityFilter, roleFilter, rawQuery, transcendLevel]);
  ...
  useEffect(() => {
    ...
    if (decoded) {
      ...
      if (decoded.role?.length) setRoleFilter(decoded.role as RoleType[]);
      if (decoded.q) setRawQuery(decoded.q);
      if (decoded.tr !== undefined) setTranscendLevel(decoded.tr);
    }
