                    ...
                    details={
                        <>
                            {lang === 'en' ? (
                                ...
                                <>
                                    <p>{lRec({ en: '', jp: 'ダメージ増加は攻撃時に与えるダメージを上昇させます。このステータスは自身の会心ダメージ（会心発生時）と', kr: '피해 증가는 공격 시 주는 데미지를 높입니다. 이 스탯은 자신의 치명타 데미지(치명 발생 시)와 ', zh: '伤害增加提升攻击时造成的伤害。该属性与自身的暴击伤害（发生暴击时）' , fr: '' } satisfies LangMap, lang)}<strong>{lRec({ en: '', jp: '加算', kr: '합산', zh: '相加' , fr: '' } satisfies LangMap, lang)}</strong>{lRec({ en: '', jp: 'され、敵の会心ダメージ減少および被ダメージ減少と共に計算されます。', kr: '되어, 적의 치명 피해 감소 및 받는 피해 감소와 함께 계산됩니다.', zh: '，并与敌方的暴击伤害减免及受到伤害减少一起计算。' , fr: '' } satisfies LangMap, lang)}</p>
                                    <p className="mt-3 font-semibold">{lRec({ en: '', jp: '仕組み：', kr: '작동 방식:', zh: '运作方式：' , fr: '' } satisfies LangMap, lang)}</p>
                                    <ul className="list-disc list-inside ml-4 mt-2">
                                        <li>{lRec({ en: '', jp: '非クリティカル時：DMG UPがこの層の唯一のボーナス。', kr: '비치명 시: DMG UP이 이 층의 유일한 보너스.', zh: '非暴击时：DMG UP是该层唯一的加成。' , fr: '' } satisfies LangMap, lang)}</li>
                                        <li>{lRec({ en: '', jp: 'クリティカル時：DMG UPが', kr: '치명 시: DMG UP이 ', zh: '暴击时：DMG UP与' , fr: '' } satisfies LangMap, lang)}{parseText('{S/CHD}')}{lRec({ en: '', jp: 'に加算され、敵の', kr: '에 합산되어 적의 ', zh: '相加后，与敌方的' , fr: '' } satisfies LangMap, lang)}{parseText('{S/DMG RED%}')}{lRec({ en: '', jp: 'と', kr: '와 ', zh: '和' , fr: '' } satisfies LangMap, lang)}{parseText('{S/CDMG RED%}')}{lRec({ en: '', jp: 'と比較されます。', kr: '와 비교됩니다.', zh: '比较。' , fr: '' } satisfies LangMap, lang)}</li>
                                    </ul>
                                </>
                            )}
                        </>
                    ...
                    details={
                        <>
                            {lang === 'en' ? (
                                ...
                                <>
                                    <p>{lRec({ en: '', jp: '被ダメージ減少は受けるダメージを割合で軽減します。この軽減はダメージ計算において防御力（DEF）', kr: '받는 피해 감소는 받는 데미지를 비율로 줄입니다. 이 감소는 데미지 계산에서 방어력(DEF) ', zh: '受到伤害减少按比例降低所有受到的伤害。该减免在伤害计算中于防御力（DEF）' , fr: '' } satisfies LangMap, lang)}<strong>{lRec({ en: '', jp: 'より前', kr: '이전에', zh: '之前' , fr: '' } satisfies LangMap, lang)}</strong>{lRec({ en: '', jp: 'に適用されます。', kr: ' 적용됩니다.', zh: '生效。' , fr: '' } satisfies LangMap, lang)}</p>
                                    <p className="mt-3 font-semibold">{lRec({ en: '', jp: '例：', kr: '예시:', zh: '示例：' , fr: '' } satisfies LangMap, lang)}</p>
                                    <p className="mt-1"><strong>{lRec({ en: '', jp: 'DMG RED 30%', kr: 'DMG RED 30%', zh: 'DMG RED 30%' , fr: '' } satisfies LangMap, lang)}</strong>{lRec({ en: '', jp: 'で1000ダメージを受けた場合、DEF計算前に', kr: '일 때 1000 데미지를 받으면, DEF 계산 전에 ', zh: '时受到1000伤害，在DEF计算前减少至' , fr: '' } satisfies LangMap, lang)}<strong>{lRec({ en: '', jp: '700', kr: '700', zh: '700' , fr: '' } satisfies LangMap, lang)}</strong>{lRec({ en: '', jp: 'に軽減されます。', kr: '으로 감소됩니다.', zh: '。' , fr: '' } satisfies LangMap, lang)}</p>
                                    <p className="mt-3 font-semibold">{lRec({ en: '', jp: 'クリティカル被弾時：', kr: '치명 피격 시:', zh: '被暴击时：' , fr: '' } satisfies LangMap, lang)}</p>
                                    <p className="mt-1">{lRec({ en: '', jp: 'DMG REDは', kr: 'DMG RED가 ', zh: 'DMG RED与' , fr: '' } satisfies LangMap, lang)}{parseText('{S/CDMG RED%}')}{lRec({ en: '', jp: 'に加算され、敵の', kr: '에 합산되어 적의 ', zh: '相加，形成对敌方' , fr: '' } satisfies LangMap, lang)}{parseText('{S/CHD}')} + {parseText('{S/DMG UP%}')}{lRec({ en: '', jp: 'に対する総防御修正値を形成します。', kr: '에 대한 총 방어 보정치를 형성합니다.', zh: '的总防御修正。' , fr: '' } satisfies LangMap, lang)}</p>
                                    <p className="mt-3 text-sm text-yellow-400"><strong>{lRec({ en: '', jp: '注意：', kr: '참고:', zh: '注意：' , fr: '' } satisfies LangMap, lang)}</strong> {lRec({ en: '', jp: 'これは最終ダメージ軽減とは異なります。計算方法が違います。', kr: '이것은 최종 데미지 감소와는 다릅니다. 계산 방식이 다릅니다.', zh: '这与最终伤害减免不同，两者的计算方式有区别。' , fr: '' } satisfies LangMap, lang)}</p>
                                </>
                            )}
                        </>
                    }
        ...
        },
        {
            ...
            content: (
                <>
                    ...
                    <p className="mt-3">{lRec({ en: 'Leading to a grand total of:', jp: '合計：', kr: '합계:', zh: '总计：' , fr: 'Pour un total de :' } satisfies LangMap, lang)} <strong>367</strong> {lRec({ en: '(477 including the speed buff)', jp: '（速度バフ込みで477）', kr: '(속도 버프 포함 시 477)', zh: '（带速度增益时477）' , fr: '(477 avec le buff SPD inclus)' } satisfies LangMap, lang)}</p>
                    <p className="mt-2">{parseText('{P/Ryu Lion}')} {lRec({ en: 'can go further with her 4 star transcend bonus:', jp: 'は4つ星超越ボーナスでさらに上げられます：', kr: '은 4성 초월 보너스로 더 올릴 수 있습니다:', zh: '的4星超越加成可进一步提升：' , fr: 'peut aller plus loin avec son bonus de transcendance 4 étoiles :' } satisfies LangMap, lang)} <strong>377</strong> {lRec({ en: '(490 with speed buff)', jp: '（速度バフ込みで490）', kr: '(속도 버프 포함 시 490)', zh: '（带速度增益时490）' , fr: '(490 avec le buff SPD)' } satisfies LangMap, lang)}</p>
                </>
            ),
        },
    ]
