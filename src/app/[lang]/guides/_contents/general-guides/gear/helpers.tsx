...
// Source: data/generated/singularity-ascension.json (regenerated by pipeline)
// ============================================================================

export const ASCENSION_INIT = {
  gold: 500_000,
  chip: 50,
  hammer: 50,
  successRate: 1.0,
...
export type AscensionStep = {
  ...
  success: number;
  factor: number;
  // Main stat gain at this step, expressed as a % of the +10 stat value.
  // Empirically derived: gain = stat_at_+10 × factor × 0.20.
  // Validated against in-game observations (T4 weapon ATK 1236→1260 at +10→+11 = +2%; SPD acc 48→49 at activation = ~+3%; SPD acc 48→55 at +15 = +15%).
  mainStatPct: number;
  unlocksBonus?: boolean;
};

export const ASCENSION_STEPS: readonly AscensionStep[] = [
  { from: 10, to: 11, gold: 300_000, chip: 10, hammer: 10, success: 0.9, factor: 0.10, mainStatPct: 0.02 },
  { from: 11, to: 12, gold: 400_000, chip: 10, hammer: 10, success: 0.8, factor: 0.10, mainStatPct: 0.02 },
  { from: 12, to: 13, gold: 500_000, chip: 10, hammer: 10, success: 0.7, factor: 0.10, mainStatPct: 0.02 },
  { from: 13, to: 14, gold: 700_000, chip: 15, hammer: 15, success: 0.6, factor: 0.10, mainStatPct: 0.02 },
  { from: 14, to: 15, gold: 1_000_000, chip: 30, hammer: 30, success: 0.4, factor: 0.20, mainStatPct: 0.04, unlocksBonus: true },
];

// Activation grants a flat +3% main stat (factor 0.15 × 0.20 = 0.03), in addition to the per-step gains above.
// Cumulative total at +15 = 0.03 + 4×0.02 + 0.04 = 0.15 (i.e. +15% vs the +10 value).
export const ASCENSION_ACTIVATION_MAIN_STAT_PCT = 0.03;
export const ASCENSION_TOTAL_MAIN_STAT_PCT = 0.15;

export const ASCENSION_REROLL = {
  gold: 500_000,
  cartridge: 10,
  successRate: 1.0,
} as const;

// Bonus effects unlocked at +15. Rate is the relative weight out of 10000.
export type Grade = 'C' | 'B' | 'A' | 'S' | 'S+';
export const GRADES: readonly Grade[] = ['C', 'B', 'A', 'S', 'S+'];

// Some effects (notably defensive elemental) yield two different ranges depending on
// the wearer's element. We model this with an optional `rangeAlt` and labels.
export type GradeRow = { range: string; rangeAlt?: string; rate: number };
...
// Standard offensive distribution (DMG, DMG vs element, DMG in Singularity)
const OFFENSIVE_RATES: Record<Grade, number> = { C: 30, B: 25, A: 25, S: 19, 'S+': 1 };
// Ultimate Skill DMG in Singularity (only 5 tiers, one per grade)
const SKILL3_RATES: Record<Grade, number> = { C: 54, B: 30, A: 11, S: 3, 'S+': 2 };
// All defensive options (DMG taken reduce, by element, in Singularity)
const DEFENSIVE_RATES: Record<Grade, number> = { C: 52, B: 26, A: 11, S: 9, 'S+': 2 };

// Group 30000 — Weapon + Accessory (offensive options)
// In-game popup grouping: 4 categories (DMG, Elemental DMG, DMG in Singularity, Ultimate in Singularity).
// The 5 elements (Fire/Water/Earth/Light/Dark) share identical ranges, so they're collapsed to one row.
export const ASCENSION_BONUS_OFFENSIVE: readonly BonusEffect[] = [
  {
    key: 'dmg', rate: 1500,
    grades: {
      C: { range: '20%-26%', rate: OFFENSIVE_RATES.C },
      B: { range: '27.5%-33.5%', rate: OFFENSIVE_RATES.B },
      A: { range: '35%-41%', rate: OFFENSIVE_RATES.A },
      S: { range: '42.5%-48.5%', rate: OFFENSIVE_RATES.S },
      'S+': { range: '50%', rate: OFFENSIVE_RATES['S+'] },
    },
  },
  {
    key: 'dmg_singular', rate: 1500,
    grades: {
      C: { range: '90%-102%', rate: OFFENSIVE_RATES.C },
      B: { range: '105%-117%', rate: OFFENSIVE_RATES.B },
      A: { range: '120%-132%', rate: OFFENSIVE_RATES.A },
      S: { range: '135%-147%', rate: OFFENSIVE_RATES.S },
      'S+': { range: '150%', rate: OFFENSIVE_RATES['S+'] },
    },
  },
  {
    key: 'dmg_skill3_singular', rate: 1000,
    grades: {
      C: { range: '100%', rate: SKILL3_RATES.C },
      B: { range: '120%', rate: SKILL3_RATES.B },
      A: { range: '140%', rate: SKILL3_RATES.A },
      S: { range: '170%', rate: SKILL3_RATES.S },
      'S+': { range: '200%', rate: SKILL3_RATES['S+'] },
    },
  },
  {
    // Pool of 5 elements (Fire / Water / Earth / Light / Dark) with identical ranges; element rolled randomly.
    key: 'dmg_to_element', rate: 6000,
    grades: {
      C: { range: '40%-52%', rate: OFFENSIVE_RATES.C },
      B: { range: '55%-67%', rate: OFFENSIVE_RATES.B },
      A: { range: '70%-82%', rate: OFFENSIVE_RATES.A },
      S: { range: '85%-97%', rate: OFFENSIVE_RATES.S },
      'S+': { range: '100%', rate: OFFENSIVE_RATES['S+'] },
    },
  },
];

// Group 31000 — Helmet + Armor + Gloves + Shoes (defensive options)
// In-game popup grouping: 3 categories (DMG Reduce, DMG Reduce in Singularity, vs Element).
// Elemental Reduce splits into two ranges: Fire/Water/Earth (strong) vs Light/Dark (weaker).
export const ASCENSION_BONUS_DEFENSIVE: readonly BonusEffect[] = [
  {
    key: 'dmg_reduce', rate: 2000,
    grades: {
      C: { range: '10%-13%', rate: DEFENSIVE_RATES.C },
      B: { range: '14.5%-17.5%', rate: DEFENSIVE_RATES.B },
      A: { range: '19%-20.5%', rate: DEFENSIVE_RATES.A },
      S: { range: '22%-23.5%', rate: DEFENSIVE_RATES.S },
      'S+': { range: '25%', rate: DEFENSIVE_RATES['S+'] },
    },
  },
  {
    key: 'dmg_reduce_singular', rate: 2000,
    grades: {
      C: { range: '45%-51%', rate: DEFENSIVE_RATES.C },
      B: { range: '54%-60%', rate: DEFENSIVE_RATES.B },
      A: { range: '63%-66%', rate: DEFENSIVE_RATES.A },
      S: { range: '69%-72%', rate: DEFENSIVE_RATES.S },
      'S+': { range: '75%', rate: DEFENSIVE_RATES['S+'] },
    },
  },
  {
    // Single 60% pool from the wearer's perspective; the displayed range depends on the
    // wearer's element (Fire/Water/Earth heroes get the strong range, Light/Dark get the weaker one).
    key: 'dmg_reduce_by_element', rate: 6000,
    splitLabels: { primary: 'F/W/E', alt: 'L/D' },
    grades: {
      C: { range: '20%-26%', rangeAlt: '15%-17%', rate: DEFENSIVE_RATES.C },
      B: { range: '29%-35%', rangeAlt: '18%-20%', rate: DEFENSIVE_RATES.B },
      A: { range: '38%-41%', rangeAlt: '21%-22%', rate: DEFENSIVE_RATES.A },
      S: { range: '44%-47%', rangeAlt: '23%-24%', rate: DEFENSIVE_RATES.S },
      'S+': { range: '50%', rangeAlt: '25%', rate: DEFENSIVE_RATES['S+'] },
    },
  },
];
...
export const ASCENSION_TOTAL_RATE = 10_000;
