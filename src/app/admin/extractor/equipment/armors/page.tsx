...
interface ArmorSetEntry {
  ...
  effect_2_1_jp?: string;
  effect_2_1_kr?: string;
  effect_2_1_zh?: string;
  effect_4_1: string | null;
  effect_4_1_jp?: string;
  effect_4_1_kr?: string;
  effect_4_1_zh?: string;
  effect_2_4: string | null;
  effect_2_4_jp?: string;
  effect_2_4_kr?: string;
  effect_2_4_zh?: string;
  effect_4_4: string | null;
  effect_4_4_jp?: string;
  effect_4_4_kr?: string;
  effect_4_4_zh?: string;
  class: string | null;
  image_prefix: string;
...
const API = '/api/admin/extractor/armor';

// ── Page ────────────────────────────────────────────────────────────

export default function ArmorExtractorPage() {
  const [entries, setEntries] = useState<ArmorSetEntry[]>([]);
  ...
  async function handleExtractAll() {
    ...
    if (!confirm(`Extract all ${entries.length} armor sets?`)) return;
  ...
  return (
    <div className="flex gap-6 h-[calc(100vh-200px)]">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        {!selectedId && !compareResult && !comparing && <div className="flex items-center justify-center h-full text-zinc-600">Select an armor set to preview, or Compare All</div>}
        {comparing && <div className="flex items-center justify-center h-full text-zinc-400">Comparing all armor sets...</div>}
        ...
        {compareResult && !selectedId && (
          <div className="space-y-3">
            ...
            {compareResult.results.length === 0 && <p className="text-sm text-green-400">All armor sets match!</p>}
        ...
        {selectedId && (() => {
          ...
          return (
            <div className="space-y-4">
              ...
              <Section title="2-Piece Effect (Tier 0)"><LangRow field="effect_2_1" data={entry as unknown as Record<string, string>} /></Section>
              <Section title="4-Piece Effect (Tier 0)"><LangRow field="effect_4_1" data={entry as unknown as Record<string, string>} /></Section>
              <Section title="2-Piece Effect (Tier 4)"><LangRow field="effect_2_4" data={entry as unknown as Record<string, string>} /></Section>
              <Section title="4-Piece Effect (Tier 4)"><LangRow field="effect_4_4" data={entry as unknown as Record<string, string>} /></Section>
            </div>
          );
        })()}
      </div>
