...
export default function WeaponExtractorPage() {
  const [entries, setEntries] = useState<WeaponEntry[]>([]);
  ...
  async function handleExtractAll() {
    ...
    if (!confirm(`Extract all ${entries.length} weapons?`)) return;
    ...
    for (let i = 0; i < ids.length; i += batch) {
      ...
      try {
        await fetch('/api/admin/extractor/weapon', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      setExtractProgress({ done, total: entries.length });
    }
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        {!selectedId && !compareResult && !comparing && (
          <div className="flex items-center justify-center h-full text-zinc-600">
            Select a weapon to preview, or Compare All
          </div>
        )}

        {comparing && (
          <div className="flex items-center justify-center h-full text-zinc-400">
            Comparing all weapons...
          </div>
        )}
        ...
        {compareResult && !selectedId && (
          <div className="space-y-3">
            ...
            {compareResult.results.length === 0 && <p className="text-sm text-green-400">All weapons match!</p>}
          ...
          return (
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                {diffEntry ? (
                  <span className="rounded bg-red-900/30 px-2 py-0.5 text-xs text-red-400">{diffEntry.diffs.length} diff(s)</span>
                ) : entry.existsInJson && compareResult ? (
                  <span className="rounded bg-green-900/30 px-2 py-0.5 text-xs text-green-400">OK</span>
                ) : !entry.existsInJson ? (
                  <span className="rounded bg-blue-900/30 px-2 py-0.5 text-xs text-blue-400">New</span>
                ) : null}
                <div className="flex-1" />
                <button onClick={() => handleSaveOne(selectedId)}
                  className="rounded-lg bg-blue-600 px-5 py-1.5 text-sm font-semibold shadow transition hover:bg-blue-500">
                  Save
                </button>
              </div>
              ...
              {/* Preview data */}
              <Section title="Name">
                <LangRow field="name" data={entry as unknown as Record<string, string>} />
              </Section>

              <Section title="Effect Name">
                <LangRow field="effect_name" data={entry as unknown as Record<string, string>} />
              </Section>

              <Section title="Effect Description (Tier 0)">
                <LangRow field="effect_desc1" data={entry as unknown as Record<string, string>} />
              </Section>

              <Section title="Effect Description (Tier 4)">
                <LangRow field="effect_desc4" data={entry as unknown as Record<string, string>} />
              </Section>
            </div>
          );
        })()}
