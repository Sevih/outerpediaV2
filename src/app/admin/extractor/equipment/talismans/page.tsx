...
const API = '/api/admin/extractor/talisman';

// ── Page ────────────────────────────────────────────────────────────

export default function TalismanExtractorPage() {
  const [entries, setEntries] = useState<TalismanEntry[]>([]);
  ...
  async function handleExtractAll() {
    ...
    if (!confirm(`Extract all ${entries.length} talismans?`)) return;
  ...
  return (
    <div className="flex gap-6 h-[calc(100vh-200px)]">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        {!selectedId && !compareResult && !comparing && <div className="flex items-center justify-center h-full text-zinc-600">Select a talisman to preview, or Compare All</div>}
        {comparing && <div className="flex items-center justify-center h-full text-zinc-400">Comparing all talismans...</div>}
        ...
        {compareResult && !selectedId && (
          <div className="space-y-3">
            ...
            {compareResult.results.length === 0 && <p className="text-sm text-green-400">All talismans match!</p>}
