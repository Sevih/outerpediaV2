...
function CharacterDetail({ id, name, rarity, exists, onSaved, initialDiffs }: {
    ...
    const fetchAll = async () => {
      try {
        ...
        if (exists) {
          ...
          if (!initialDiffs) {
            const compareRes = await fetch('/api/admin/extractor-v2?action=compare');
          ...
          const [infoRes, skillsRes, transcendRes] = await Promise.all([
            fetch(`/api/admin/extractor-v2?action=info&id=${id}`),
            fetch(`/api/admin/extractor-v2?action=skills&id=${id}`),
            fetch(`/api/admin/extractor-v2?action=transcend&id=${id}`),
          ]);
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/admin/extractor-v2', {
        method: 'POST',
