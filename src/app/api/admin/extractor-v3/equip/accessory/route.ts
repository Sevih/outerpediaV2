...
function buildAccessoryBossMap(t: EquipTables): Map<string, string | string[]> {
  const map1 = buildBossMap(t, 'DM_RAID_1')
  const map2 = buildBossMap(t, 'DM_RAID_2')
  for (const [k, v] of map2) { if (!map1.has(k)) map1.set(k, v) }
  return map1
}
...
export async function GET(req: NextRequest) {
  ...
  const bossMap = buildAccessoryBossMap(t)
