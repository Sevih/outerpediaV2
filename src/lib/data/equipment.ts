...
async function getStatRanges(): Promise<Record<string, unknown>> {
  if (!statRangesCache) {
    statRangesCache = await readJson(join(EQUIP_DIR, 'statRanges.json'));
  }
...
export async function getWeapons(): Promise<Weapon[]> {
  return readJson<Weapon[]>(join(EQUIP_DIR, 'weapon.json'));
}

export async function getAmulets(): Promise<Amulet[]> {
  return readJson<Amulet[]>(join(EQUIP_DIR, 'accessory.json'));
}

export async function getTalismans(): Promise<Talisman[]> {
  const data = await readJson<Talisman[]>(join(EQUIP_DIR, 'talisman.json'));
  for (const t of data) t.level = Number(t.level);
  return data;
}

export async function getArmorSets(): Promise<ArmorSet[]> {
  return readJson<ArmorSet[]>(join(EQUIP_DIR, 'sets.json'));
}

export async function getExclusiveEquipment(): Promise<Record<string, ExclusiveEquipment>> {
  return readJson<Record<string, ExclusiveEquipment>>(join(EQUIP_DIR, 'ee.json'));
}
...
export async function getCharactersRecommendingEquipment(
  ...
  const presets = await readJson<RecoPresets>(join(RECO_DIR, '_presets.json'));
  ...
  const recoEntries = await Promise.all(
    recoFiles.map(async (file) => {
      ...
      return { charId, reco: await readJson<CharacterReco>(join(RECO_DIR, file)) };
    })
  );
...
export async function getGearFinderData(): Promise<GearFinderBuild[]> {
  return readJson<GearFinderBuild[]>(join(GENERATED_DIR, 'gear-finder-index.json'));
}
