"""Full dump of FX_UI_Character_List_Dungeon prefab.
Walks every GameObject, dumps every property of every component.
"""
import UnityPy
import sys
...
def is_zero(v):
    """Recursively check whether a value is all-zero (or empty)."""
...
def dump_value(v, indent, depth=0):
    if depth > 5:
        ...
        return
    if isinstance(v, dict):
        # Compact form for short dicts (vector, color, simple struct)
        keys = list(v.keys())
        ...
        for k, val in v.items():
            ...
            if isinstance(val, (dict, list)) and val:
                # Special: print scalar/curve compactly if it has a 'scalar' key
    ...
    if t in ('RectTransform', 'Transform'):
        ...
        for k in ['lengthInSec', 'simulationSpeed', 'looping', 'prewarm',
            ...
            if k in comp and not is_zero(comp[k]):
                print(f'{indent}{k}: {fmt_val(comp[k])}')
        # Modules — every dict-typed key that has 'enabled' or content
...
print('=' * 70)
print('FULL DUMP: fx_ui_character_list_dungeon')
print('=' * 70)
